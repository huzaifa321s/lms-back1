...
const gameCategoryController = {
  ...
  try {
    ...
    const gameCategories = await GameCategory.aggregate([
      ...
      {
        $lookup: {
          from: "trainingwheelgames", // Must match your collection name
          localField: "_id",
          foreignField: "category",   // Must match your model field
          as: "games",
        },
      },
      {
  ...
  try {
    ...
    const [result] = await GameCategory.aggregate([
      ...
      {
        $lookup: {
          from: "trainingwheelgames", // Correct collection name from your model
          localField: "_id",
          foreignField: "category",   // Field name as per your TrainingWheelGame model
          as: "games",
        },
      },
      {
